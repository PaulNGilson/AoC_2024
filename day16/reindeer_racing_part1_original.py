...
bests = {}
current_paths = [(start, "E", 0)] # coord, direction, cost
while current_paths:
    ...
    if path[1] == "E":
        if can_move(path[0], "E"):
            new_paths.append(((path[0][0]+2, path[0][1]), "E", path[2]+2))
        if can_move(path[0], "N"):
            new_paths.append(((path[0][0], path[0][1]-2), "N", path[2]+1002))
        if can_move(path[0], "S"):
            new_paths.append(((path[0][0], path[0][1]+2), "S", path[2]+1002))
    elif path[1] == "W":
        if can_move(path[0], "W"):
            new_paths.append(((path[0][0]-2, path[0][1]), "W", path[2]+2))
        if can_move(path[0], "N"):
            new_paths.append(((path[0][0], path[0][1]-2), "N", path[2]+1002))
        if can_move(path[0], "S"):
            new_paths.append(((path[0][0], path[0][1]+2), "S", path[2]+1002))
        ...
        if can_move(path[0], "N"):
            new_paths.append(((path[0][0], path[0][1]-2), "N", path[2]+2))
        if can_move(path[0], "W"):
            new_paths.append(((path[0][0]-2, path[0][1]), "W", path[2]+1002))
        if can_move(path[0], "E"):
            new_paths.append(((path[0][0]+2, path[0][1]), "E", path[2]+1002))
    elif path[1] == "S":
        if can_move(path[0], "S"):
            new_paths.append(((path[0][0], path[0][1]+2), "S", path[2]+2))
        if can_move(path[0], "W"):
            new_paths.append(((path[0][0]-2, path[0][1]), "W", path[2]+1002))
        if can_move(path[0], "E"):
            new_paths.append(((path[0][0]+2, path[0][1]), "E", path[2]+1002))
    for new_path in new_paths:
        if new_path[0] in bests.keys():
            if new_path[1] in bests[new_path[0]].keys():
                if new_path[2] < bests[new_path[0]][new_path[1]]:
