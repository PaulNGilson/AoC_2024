...
#         path[steps+1] = (current[0], current[1]-1)
#         path[steps+2] = (current[0], current[1]-2)
...
#         path[steps+1] = (current[0]-1, current[1])
#         path[steps+2] = (current[0]-2, current[1])
...
# for step in range(0, steps-(amount-1)):
#     jump_to = step+amount
#     while jump_to in path.keys():
#         manhattan = abs(path[step][0]-path[jump_to][0]) + abs(path[step][1]-path[jump_to][1])
#         step_gain = jump_to - step - manhattan
#         if step_gain >= 100 and manhattan <= amount:
#             #print(step, jump_to, manhattan, step_gain)
#             count_part1 += 1
#         jump_to += 1
...
def get_reachable_variable_amount(x, y, amount):
    reachables = []
    # produce a more compact "diamond" of cells in range of the amount
    for yr in range(-amount, amount+1):
        for xr in range(-amount+abs(yr), amount+1-abs(yr)):
